type State = '.' | 'L' | '#';
type Seats = Array< Array< State > >;
type NearFunction = ( seats: Seats, x: number, y: number ) => number;

const serialize = ( seats: Seats ) =>
	seats.map( row => row.join( '' ) ).join( '\n' );

const countOccupied = ( seats: Seats ) =>
	seats.reduce(
		( prev, cur ) =>
			prev +
			cur.reduce( ( prev, cur ) => prev + ( cur === '#' ? 1 : 0 ), 0 ),
		0
	);

function adjacents( seats: Seats, x: number, y: number ): number
{
	const w = seats[ 0 ].length, h = seats.length;
	let count = 0;
...
function seeable( seats: Seats, x: number, y: number ): number
{
	const w = seats[ 0 ].length, h = seats.length;

	function look( dx: number, dy: number ): State
	{
		let i = dx, j = dy;
		while ( x + i >= 0 && y + j >= 0 && x + i < w && y + j < h )
		{
			if ( seats[ y + j ][ x + i ] !== '.' )
				return seats[ y + j ][ x + i ];
			i += dx;
			j += dy;
		}
		return '.';
...
function solve( seats: Seats, near: NearFunction, limit: number )
{
	const sit = ( seats: Seats ): Seats =>
		seats.map( ( row, y ) =>
			row.map( ( seat, x ) =>
				( seat === 'L' && near( seats, x, y ) === 0 )
				? '#' : seat
			)
		);

	const leave = ( seats: Seats ): Seats =>
		seats.map( ( row, y ) =>
			row.map( ( seat, x ) =>
				( seat === '#' && near( seats, x, y ) >= limit )
				? 'L' : seat
			)
		);
	...
	seats = sit( seats );
	let last: string;
	while ( last !== serialize( seats ) )
	{
		last = serialize( seats );
		seats = leave( seats );
		seats = sit( seats );
	}
...
console.log( "(1) Seat count", solve( data, adjacents, 4 ) );
console.log( "(2) Seat count", solve( data, seeable, 5 ) );

function parse( data: string ): Seats
{
	return data
	.trim( )
	.split( "\n" )
	.map( line => line.trim( ).split( '' ) )
	.map( line => line as Array< State > );
}
