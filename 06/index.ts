...
function parseAnyone( data: string )
{
	return data
		.trim( )
		.split( "\n\n" )
...
function parseEveryone( data: string )
{
	return data
		.trim( )
		.split( "\n\n" )
			...
			.forEach( answers =>
			{
				...
				.forEach( char =>
				{
					if ( answerMap.has( char ) )
						answerMap.set( char, answerMap.get( char ) + 1 );
					else
						answerMap.set( char, 1 );
				} );
			} );

			return [ ...answerMap.entries( ) ]
				.reduce(
					( prev, cur ) =>
						prev + ( cur[ 1 ] === numPeople ? 1 : 0 )
					,
					0
				);
		} );
...
console.log( count( parseAnyone( getMyData( ) ) ) );
console.log( count( parseEveryone( getMyData( ) ) ) );
