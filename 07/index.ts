...
type BagTree = Map< string, Bag[ 'contains' ] >;
type BagReverseTree = Map< string, Array< string > >;


const requirement = getRequirement( getMyData( ) );
const tree = getTree( requirement );
const reverseTree = getReverseTree( requirement );

console.log( "Can contain gold", getHolders( 'shiny gold' ).length );
console.log( "Gold must carrry", getSubBags( 'shiny gold' ) );

function getSubBags( leaf: string )
{
	const recurse = ( bag: string, mul: number ): number =>
	{
		const contains = tree.get( bag );
		return ( contains ?? [ ] ).reduce(
			( prev, { type, amount } ) =>
				prev + amount * mul + recurse( type, mul * amount ),
			0
		);
	};

	return recurse( leaf, 1 );
}

function getHolders( leaf: string )
{
	const holders = new Set< string >( );

	const recurse = ( bag: string ) =>
	{
		const parents = reverseTree.get( bag );
		( parents ?? [ ] ).forEach( parent =>
		{
			if ( !holders.has( parent ) )
				recurse( parent );
			holders.add( parent );
		} );
	};
...
function getTree( requirement: Array< Bag > ): BagTree
{
	return new Map(
		requirement.map( ( { type, contains } ) => [ type, contains ] )
	);
}

function getReverseTree( requirement: Array< Bag > ): BagReverseTree
{
	const map = new Map( ) as BagReverseTree;

	requirement.forEach( ( { type, contains } ) =>
	{
		contains.forEach( contain =>
		{
			if ( map.has( contain.type ) )
				map.get( contain.type ).push( type );
			else
				map.set( contain.type, [ type ] );
		} );
	} );
...
function getRequirement( data: string ): Array< Bag >
{
	return data
		.trim( )
		.split( "\n" )
		...
		.map( ( line ): Bag =>
		{
			...
			if ( m[ 2 ] === 'no other bags.' )
				...
				return {
					...
					.map( text =>
					{
						const m = text.match( /^(\d+) (.*) bags?,?.?$/ );
						return {
							type: m[ 2 ],
							amount: parseInt( m[ 1 ] ),
						} as Bag[ 'contains' ][ number ];
					} )
				};
		} );
}
