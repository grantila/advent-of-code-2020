const required: [ field: string, validator: ( data: string ) => boolean ][ ] = [
	// (Birth Year)
	[
		'byr',
		data => parseInt( data ) >= 1920 && parseInt( data ) <= 2002
	],
	// (Issue Year)
	[
		'iyr',
		data => parseInt( data ) >= 2010 && parseInt( data ) <= 2020
	],
	// (Expiration Year)
	[
		'eyr',
		data => parseInt( data ) >= 2020 && parseInt( data ) <= 2030
	],
	// (Height)
	[
		'hgt',
		data =>
		{
			const m = data.match( /^(\d+)((?:cm)|(?:in))$/ )
			if ( !m )
				return false;
			const height = parseInt( m[ 1 ] );
			return ( m[ 2 ] === 'cm' && height >= 150 && height <= 193 )
				|| ( m[ 2 ] === 'in' && height >= 59 && height <= 76 );
		}
	],
	// (Hair Color)
	[
		'hcl',
		data => !!data.match( /^#[0-9a-f]{6}$/ ),
	],
	// (Eye Color)
	[
		'ecl',
	...
	[
		'pid',
		data => !!data.match( /^[0-9]{9}$/ ),
	],
];

function go( data: string )
{
	const passports = data
		...
		.filter( passport =>
			!required.some( ( [ field, validator ] ) =>
				!passport.has( field )
				||
				!validator( passport.get( field ) )
			)
		)
		.length;
